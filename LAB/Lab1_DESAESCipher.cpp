...
int selectScheme()
{
	...
	wcout << L"Option: ";
	wcin >> schemeNumber;
	while (schemeNumber != 1 && schemeNumber != 2)
		{
			wcout << L"Option unvailable. Please choose different mode: " << endl;
			wcin >> schemeNumber;
		}
	return schemeNumber;
}
...
int selectMode(bool check_AES)
{
	...
	int modeNumber;
	wcin >> modeNumber;
	while (modeNumber < 1 || (modeNumber > 8 && check_AES) || (modeNumber > 5 && !check_AES))
		{
			wcout << L"Option unvailable. Please choose different mode: " << endl;
			wcin >> modeNumber;
		}
...
int selectKeySize(int modeNumber)
{
	...
	try
	{
		...
		{
			wcout << L"Option unvailable. Please choose again!" << endl;
			return selectKeySize(modeNumber);
		}
	}
	catch (const std::exception &e)
	{
		wcout << L"Option unvailable. Failed to executed. Exiting..." << endl;
		exit(1);
	}
}
...
{
	wcout << L"Choose IV size: " << endl;
	wcout << L"1. Manually input" << endl;
	wcout << L"2. Default IV size" << endl;
	wcout << L"Option: ";
	int IVSizeNumber, optionNumber;
	try
	{
		wcin >> optionNumber;
		while (optionNumber < 1 or optionNumber > 2)
		{
			wcout << L"Option is unvailable. Please select again: " << endl;
			wcin >> optionNumber;
		}
		if (optionNumber == 1)
		{
			...
			{
				wcout << L"Choose IV size [7,13]: " << endl;
				wcin >> IVSizeNumber;
				if (IVSizeNumber < 7 || IVSizeNumber > 13)
				{
					wcout << L"Option is unvailable" << endl;
					exit(1);
				}
			}
		}
...
void getDataFromConsole(SecByteBlock &block, int blockSize, wstring type)
{
	...
	try
	{
		...
		wstring wDataInput;
		wcout << L"Provide your input: " << type << L": ";
		fflush(stdin);
...
{
	wcout << L"Provide " << type << L" randomly or read from file: \n";
	wcout << L"1. Provide input" << endl;
	wcout << L"2. Random input" << endl;
	wcout << L"3. From File" << endl;
	wcout << L"Option: ";
	int optionNumber;
	try
	{
		...
		{
			wcout << L"Option is not available. Please try again: " << endl;
			wcin >> optionNumber;
		}
		if (optionNumber == 1)
		{
		...
		{
			wcout << L"Option is not available" << endl;
			exit(1);
		}
	}
	catch (const std::exception &e)
...
void decrypt(const string &ciphertext, mode &dec, string &plaintext)
{
	...
	{
		wcout << L"Failed decryption" << endl;
		exit(1);
	}
}
...
int main(int argc, char *argv[])
{
    while (1)
    {
        ...
        else if (choice == 2)
        {
			...
			ifstream file(ws2s(filename));
			if (!file)
			{
				wcout << L"Error: Cannot open file " << filename << endl;
        ...
        else if (scheme == 2)
        {
            ...
            {
                wcout << L"keySize 64 bit is not available in this mode" << endl;
                exit(1);
            }
        ...
        {
            wcout << L" Unavailable scheme" << endl;
            exit(1);
        }
        ...
        wcout << L"Keep data in file?" << endl;
        wcout << L"1. Yes" << endl;
        wcout << L"2. No" << endl;
        wcout << L"Option: ";
        int choiceFile;
        ...
        if (choiceFile == 1)
        {
            ...
            outFile << "--------------------------------------------------------------------------------" << endl;
            outFile.close();
            wcout << L"The function is successfully executed" << endl;			
        }
        else
        {
            wcout << L"Failed" << endl;
