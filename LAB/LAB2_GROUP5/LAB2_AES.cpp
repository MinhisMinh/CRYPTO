...
int main(int argc, char* argv[])
{
    ...
    int inputChoice;
    wcout << L"Chọn phương thức nhập dữ liệu:" << endl;
    wcout << L"1. Nhập từ màn hình" << endl;
    wcout << L"2. Đọc từ file" << endl;
    wcout << L"Lựa chọn: ";
    wcin >> inputChoice;
    ...
    if (inputChoice == 1)
    {
        wcout << L"Nhập vào chuỗi cần mã hóa: ";
        wcin >> wplaintext;
    ...
    {
        wstring filename;
        wcout << L"Nhập tên file chứa plaintext: ";
        wcin >> filename; 
        string filepath = ws2s(filename);
        ifstream file(filepath);
        if (!file)
        {
            wcout << L"Lỗi: Không thể mở file " << filename << endl;
    ...
    auto encryptTime = std::chrono::duration_cast<std::chrono::milliseconds>(endEncrypt - startEncrypt).count();
    double avgEncryptTime = static_cast<double>(encryptTime) / 10000; // Calculate average encryption time

    auto ciphertext = encrypt(plaintext, key, iv); // Encrypt once to get ciphertext for decryption

    auto startDecrypt = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < 10000; ++i)
    {
        auto recovertext = decrypt(ciphertext, key, iv);
    }
    auto endDecrypt = std::chrono::high_resolution_clock::now();
    auto decryptTime = std::chrono::duration_cast<std::chrono::milliseconds>(endDecrypt - startDecrypt).count();
    double avgDecryptTime = static_cast<double>(decryptTime) / 10000; // Calculate average decryption time
    ...
    // Reset to decimal format before printing time
    wcout << std::dec;

    wcout << L"Total encryption time for 10,000 iterations: " << encryptTime << " ms" << endl;
    wcout << L"Average encryption time per iteration: " << avgEncryptTime << " ms" << endl;

    wcout << L"Total decryption time for 10,000 iterations: " << decryptTime << " ms" << endl;
    wcout << L"Average decryption time per iteration: " << avgDecryptTime << " ms" << endl;
    return 0;
}